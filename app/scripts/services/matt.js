...
angular.module('HarvardApp')
    .service('Matt', ['$http', '$timeout', '$log', '$location', '$sce', 'moment', function Matt($http, $timeout, $log, $location, $sce, moment) {
        ...
        return {
            ...
            cutQuantityValidation: cutQuantityValidation,
...
.controller('SoftleaderCtrl', ['$scope', 'Matt', '$alert', function ($scope, Matt, $alert) {
    ...
    $scope.unCutModal = function() {
        console.log($scope.cutModalTask);
        $scope.cutModalTask.model.cut = false;
        $scope.cutModalTask.model.cutQuantity = 0;
        $scope.cutModal.hide();
        $scope.cutModal = undefined;
    };
    $scope.saveCutModal = function() {
        var result = Matt.cutQuantityValidation({ cutQuantity: $scope.taskCutQuantity });
        if (result.state === 'ok') {
            $scope.cutModalTask.model.cut = $scope.taskCutStatus === '1' ? true : false;
            $scope.cutModalTask.model.cutQuantity = parseInt(angular.copy($scope.taskCutQuantity), 10);
            $scope.cutModal.hide();
            $scope.cutModal = undefined;
        } else {
            $alert({
                ...
                placement: 'top',
                type: 'info',
                duration: Matt.configuration.alertTimeout,
                dismissable: true,
                html: true,
