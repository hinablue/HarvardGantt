...
(function(){
    'use strict';
    angular.module('gantt').factory('GanttRowsManager', ['GanttRow', 'ganttArrays', '$filter', '$timeout', '$log', 'moment', function(Row, arrays, $filter, $timeout, $log, moment) {
                    ...
                    if (newTasks !== oldTasks) {
                        ...
                        if (newTasks !== undefined) {
                            for (i= 0, l = newTasks.length; i<l; i++) {
                                var toAdd = row.tasksMap[newTasks[i].id].model;
                                row.addTask(toAdd);
                            }
                        }
                    }
            ...
            if (filterRow) {
                ...
                if (typeof(filterRowComparator) === 'function') {
                    filterRowComparator = function(actual, expected) {
                        actual = actual.model === undefined ? actual : actual.model;
                        expected = expected.model === undefined ? expected : expected.model;
...
angular.module('gantt.templates', []).run(['$templateCache', function($templateCache) {
    $templateCache.put('template/gantt.tmpl.html',
        ...
        '            <div context-menu class="gantt-context-menu" data-target="taskmenu-{{task.model.id}}"></div>\n' +
        ...
        '            <div class="gantt-task-content" ng-style="{color: \'{{task.model.textColor}}\'}">\n' +
        '                <div class="gantt-task-content-templ" ng-include="task.model.taskContent"></span><span class="middle-placeholder"></div>\n' +
        '            </div>\n' +
        '        </div>\n' +
        '    </script>\n' +
        '\n' +
